...
# get the x parametrization of a concentration vector
x_logphi = lambda phi: jnp.log(phi)

# the inverse transformation of `get_x_from_phi`; get the phi parametrization of a x-parametrized concentration vector
phi_expx = lambda x: jnp.exp(x)

#--------------------Model A ODE RHS--------------------#

def rhs(t, phi_p, xp, args):
    '''
    Description: obtain the RHS of Model-A dynamics (formatted for Diffrax)
    ...
    '''
    phi_in, chi, mu_res, d_sol = args

    d_vec = phi_p * jnp.array([*jnp.ones(len(phi_p)-1), d_sol])

    chi_term = jnp.array([*jnp.matmul(chi, jnp.concatenate([phi_in, phi_p[:-1]])), 0])
    mu_x = 1 + xp + chi_term
    mu_diff = mu_x - jnp.array([*mu_res, 0])
    return - d_vec * (mu_diff - jnp.dot(d_vec, mu_diff)/jnp.sum(d_vec))
...
def phi_ODE(t, phi_p, args):
    ...
    return jacobian * rhs(t, phi_p, xp, args)

def x_ODE(t, xp, args):
    '''
    Description: wrapper for the RHS of Model-A dynamics using parametrization x=log(phi) (formatted for Diffrax)

    Parameters:
        xp (array): concentration vector parametrized as x=log(phi) for output and hidden concentrations
        args (tuple):
            phi_in (array): vector of input concentrations (unparametrized)
            chi (array): interaction matrix
            mu_res (array): chemical potential of reservoir
...
def get_integrator(d_sol=1., int_param='x', illustrate=False):
    ...
    def diffrax_integrate(phi_p, phi_in, chi_p, mu_res_p):
        ...
        '''
        # Parametrization
        phi_s = 1. - jnp.sum(phi_in) - jnp.sum(phi_p)
        phi_p = jnp.array([*phi_p, phi_s]) # Solvent concatenated to last position
        if int_param=='x':
            x0 = x_logphi(phi_p)
        else:
            x0 = phi_p
        ...
        solution = diffrax.diffeqsolve(term, 
                            ...
                            dt0=dt0,
                            y0=x0,
                            args=(phi_in, chi_p, mu_res_p, d_sol),
        ...
        if int_param == 'x':
            phi_final = phi_expx(solution.ys[-1])#[:-1]
            phi_aux = phi_expx(solution.ys)#[:,:-1]
        else:
            phi_final = (solution.ys[-1])[:-1]
            phi_aux = (solution.ys)[:,:-1]
        t_vals = solution.ts
